...
### ADR-005：前端命名为 hifi-web

**决策：** 前端模块命名为 `hifi-web`。

**原因：** 后端模块前缀统一 `hify-`，前端命名不再套用此前缀，避免 `hify-frontend` 显得臃肿。`hifi` 取项目名 Hify 的前半部分，`-web` 明确表示 Web 前端，和 `hify-app`（后端启动）形成前后端对称。
