...
| **hifi-web** | Deployment | 1 | 128Mi / 256Mi | Nginx 镜像，挂载 Vue 静态文件 |
...
Nginx Ingress (TLS 终止)
    │ path: /          → hifi-web:80
    ▼
hifi-web (Nginx 容器，serve Vue 静态文件)
...
**为什么不是 Nginx 反代动态请求？** hifi-web 只做静态资源，不代理 `/api`。所有 `/api/*` 由 Ingress 直接路由到 hify-app。这样前端容器就是纯文件服务器，无任何动态逻辑。
...
|---|---|---|---|---|
| hifi-web | 1 | 50m / 200m | 128Mi / 256Mi | - |
